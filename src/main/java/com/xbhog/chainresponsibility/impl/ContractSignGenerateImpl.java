 ...
 */
@Slf4j
@ContractSign(SIGN_CHANNEL = ContractSignEnum.SignChannel.SIGN_GENERATE)
@Component
public class ContractSignGenerateImpl<T extends ContractRequest> implements Interceptor<T , ContractResponse> {

    @Override
    public ContractResponse process(Chain<T, ContractResponse> chain) {
        log.info("=============执行合同文本生成拦截器开始");
        //获取处理的请求参数
        T request = chain.request();
        log.info("=============执行合同文本生成拦截器结束");
        //进入下一个责任链节点
